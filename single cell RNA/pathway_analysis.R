# load the DEG data
dea<-read.csv("ACM_coculture_vs_ACM_DEA.csv",row.names = 1)
...
up_down_reg <- rbind(up_reg, down_reg)
up_down_reg <- up_down_reg[order(up_down_reg$p.val,decreasing = FALSE),]
...
ggplot(up_down_reg[1:10,], aes(x=p.val,y=reorder(description,-log10(p.val)),color=p.val,size=set.size)) + 
  geom_point()+
  scale_color_gradient(low = "red", high = "blue") + 
  theme_bw() + 
  xlab('set.size')+
