...
data_input <- GetAssayData(SAN_PCO,layer = "data",assay = "RNA")


meta <- data.frame(labels = Idents(SAN_PCO),row.names = names(Idents(SAN_PCO)))
...
netVisual_bubble(cellchat, sources.use = -9, targets.use = 9, remove.isolate = FALSE,grid.on = FALSE) + coord_flip()
netVisual_bubble(cellchat, sources.use = -1, targets.use = 1, remove.isolate = FALSE,grid.on = FALSE) + coord_flip()

netVisual_bubble(cellchat, sources.use = -9, targets.use = 9, remove.isolate = FALSE,grid.on = TRUE)
...
saveRDS(cellchat, file = "cellchat_SAN_PCO.rds")
...
netVisual_bubble(cellchat,
                 sources.use = 9,  # From cell types (sources)
