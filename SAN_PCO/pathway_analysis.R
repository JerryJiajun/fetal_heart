# load the DEG data
dea<-read.csv("CM_coculture_vs_mixture.csv",row.names = 1)
# dea<-read.csv("SAN_coculture_vs_mixture.csv",row.names = 1)
...
ggplot(up_reg[1:15,], aes(x=p.val,y=reorder(description,-log10(p.val)),color=p.val,size=set.size)) + 
  geom_point()+
