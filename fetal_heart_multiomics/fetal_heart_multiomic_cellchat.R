...
data_input <- GetAssayData(fetal_heart,layer = "data",assay = "RNA")
...
df.net <- subsetCommunication(cellchat, sources.use = 13, targets.use = 2)
...
netVisual_bubble(cellchat, sources.use = c(-11), targets.use = 11, remove.isolate = FALSE)
netVisual_bubble(cellchat, sources.use = 11, targets.use = c(-11), remove.isolate = FALSE)

netVisual_bubble(cellchat, sources.use = c(-3), targets.use = 3, remove.isolate = FALSE)
netVisual_bubble(cellchat, sources.use = 3, targets.use = c(-3), remove.isolate = FALSE)

netVisual_bubble(cellchat, sources.use = 11, targets.use = 1, remove.isolate = FALSE)
netVisual_bubble(cellchat, sources.use = 1, targets.use = 13, remove.isolate = FALSE)
netVisual_bubble(cellchat, sources.use = 8, targets.use = 11, remove.isolate = FALSE)
...
df.comm_Neuron_SAN <- df.comm[df.comm$source == 'Neuronal_1' & df.comm$target == 'SAN',]
...
netVisual_bubble(cellchat,
                 ...
                 targets.use = 11,    # To cell types (targets)
                 signaling = top_interactions$pathway_name,
                 remove.isolate = TRUE,
                 grid.on = FALSE)


saveRDS(cellchat, file = "cellchat_fetal_heart_multiomics.rds")
